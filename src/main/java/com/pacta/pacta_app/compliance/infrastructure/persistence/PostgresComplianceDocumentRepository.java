...
public class PostgresComplianceDocumentRepository implements IComplianceDocumentRepository {
    ...
    @Override public ComplianceDocument update(ComplianceDocument d) { jpa.saveAndFlush(toEntity(d)); return d; }
    ...
    public Optional<ComplianceDocument> findByUserIdAndTypeCode(String userId, String typeCode) {
        return jpa.findByUserIdAndDocumentType(userId, typeCode).map(this::toDomain);
    }

    private ComplianceDocumentJpaEntity toEntity(ComplianceDocument d) {
        return ComplianceDocumentJpaEntity.builder()
                .id(d.getId()).userId(d.getUserId()).documentType(d.getTypeCode())
    ...
    private ComplianceDocument toDomain(ComplianceDocumentJpaEntity e) {
        return ComplianceDocument.builder()
                .id(e.getId()).userId(e.getUserId()).typeCode(e.getDocumentType())
